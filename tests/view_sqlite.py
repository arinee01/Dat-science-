# -*- coding: utf-8 -*-
"""
Script to view data from the SQLite database relational.db
"""
...
def view_sqlite_data():
    """View data in the SQLite database"""

    db_path = ".." + os.sep + "relational.db"

    print("=== Viewing SQLite database data ===\n")
...
def test_category_query_handler():
    ...
    print("Fetching all categories via our handler:")
    df = handler.getAllCategories()
    print(f"   Categories fetched: {len(df)}")
    ...
    # Test fetching all areas
    print("\nFetching all areas via our handler:")
    df = handler.getAllAreas()
    print(f"   Areas fetched: {len(df)}")
    ...
    # Test searching by quartiles
    print("\nSearch for categories with quartile Q1:")
    df = handler.getCategoriesWithQuartile({"Q1"})
    print(f"   Found Q1 categories: {len(df)}")

    if not df.empty:
        print("   First 5 Q1 categories:")
        for i, (_, row) in enumerate(df.head(5).iterrows()):
            print(f"   {i+1}. {row.get('id', 'N/A')}")


def view_journal_categories():
    """Show relationships between journals and categories"""
    ...
    try:
        ...
        if not df.empty:
            print("Sample journal-category-area relations:")
        ...
        print(f"Error while viewing relations: {e}")
